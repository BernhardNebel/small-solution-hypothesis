...
# the claims in the paper "The Small Solution Hypothesis for MAPF on Strongly Connected Directed Graphs Is True."
...
def t0cycle_claim():
    ...
    for (n1, n2, n3) in [(1,3,2), (2,2,2), (1,2,3)]:
        ...
        if (n1, n2, n3) not in [(1,3,2), (2,2,2)]:
...
def pairtype(c1, c2):
    ...
    if len(c1) > len(c2):
        c1, c2 = c2, c1
    if sharednodes(c1,c2) < 1:
        return None
    else:
        return (len(c1) - sharednodes(c1,c2) - 1, sharednodes(c1,c2),
...
def gen_t0pair_ext():
    ...
    print("========================================================")
    t0adict = {  'a1' : [  'b3' ], 'b3': ['b2'], 'b2': [ 'b1' ], 
        'b1' : ['a1', 'c1'], 'c1': ['c2'], 'c2': ['b3'] }
    t0bdict = { 'a1': ['a2'], 'a2' : [ 'b2' ], 'b2': [ 'b1' ], 
        'b1' : ['a1', 'c1'], 'c1': ['c2'], 'c2': ['b2'] }
    ears = [ ['e1'], ['e1', 'e2'] ]
    tested = []

    for graph, ptype in ( (t0adict, (1,3,2)), (t0bdict, (2,2,2)) ):
        for head in graph.keys():
            for tail in graph.keys():
                for ear in ears:
                    t0 = DiGraph(graph)
                    t0.add_path([ head ] + ear + [ tail ])
                    if all([not t0.is_isomorphic(g) for g in tested]):
                        tested += [t0]
...
def t0_ext_claim():
    ...
    # (b) case a_2 to b_1 for (2,2,2)
    a = G((1,2,3,4))
    b = G((1,2,5,6))
    d = G((3,4,8,7))
    p = b^-1*d^-1*a^-1*b^2*d^-1*a^-1*d^-1
    print("Case b:",p) 
    assert len(p.cycle_type(False)) == 1 and p.cycle_type(False)[0] == 3
